// src/pages/MobileLayout.tsx - Complete Implementation with All Pages
...
// --- PROFILE PAGE ---
const ProfilePage = ({ onThemeToggle, currentTheme }: { onThemeToggle: () => void, currentTheme: 'light' | 'dark' }) => {
  const { user } = useUser();
  ...
  return (
    <div className="mobile-page-content">
      ...
      <div className="settings-section">
        ...
        <button className="settings-item" onClick={onThemeToggle}>
          {currentTheme === 'light' ? <Moon size={20} /> : <Sun size={20} />}
          <span>Theme</span>
          <span className="settings-value">{currentTheme === 'light' ? 'Light' : 'Dark'}</span>
        </button>
  ...
  const renderContent = () => {
    switch (activeTab) {
      ...
      case 'budgets':
        return <BudgetsPage />;
      case 'profile':
        return <ProfilePage onThemeToggle={toggleTheme} currentTheme={theme} />;
      default:
        return <HomePage />;
    }
  };
